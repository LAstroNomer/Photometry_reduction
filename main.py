...
def main(fnames, bdr=0, low_mag=14, up_mag=17, fwhm=9.0, k0=2, k1=2.5, k2=3, 
            path_to_mask=None, catalog='NOMAD', manual=True, exptime=None, filters='BVR', out_dir='.', app=None):
    ...
    if len(fnames) == 1:
        ...
        xs = [x[0] for x in Bxy]
        ys = [x[1] for x in Bxy]
        ufoo.save([inds, xs, ys,  Best, Bmag, Bfwhm], ['index', 'x_center', 'y_center', 'B_instrumental', 'B_catalog', 'FWHM'], out_dir) 
    
        mps.get_equals_solo(cat_B=Bmag, est_B=Best, fnameB=fnames[0], filt=filters[0], out_dir=out_dir, inds0=inds)
        return
